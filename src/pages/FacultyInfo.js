...
const FacultyInfo = () => {
  ...
  useEffect(() => {
    setFaculty(location.state.faculty); // result: 'some_value'
  }, [location]);
  ...
  return (
    <div className="pl-40 py-14 flex flex-col space-y-7">
      ...
      <div className="flex flex-row space-x-2 mx-auto py-5 px-12 w-1/2 justify-between items-center bg-white shadow-xl rounded-lg">
        <div className="flex flex-col">
          <p className="text-gray-400 text-xs ">Give your Feedback</p>
          <input
            type="text"
            value={feedback}
            onChange={(e) => setFeedback(e.target.value)}
            className="w-96 text-md text-gray-600  font-medium  focus:outline-none"
          />
        </div>
        <button
